* Overview
Rewrite at the end of 2022, beginning of 2023. Time to evaluate what I still
need and try out some new packages.

This file isn't used, I'm going back to a vanilla init for awhile.

* Basics
** GC Cons Threshhold

I've bumped this up mainly because of lsp-mode performance suggestions. I may
turn it off entirely during init in order to get a faster start.
...
This is highly specific to my machine, but I rely on it to get to my config.
  #+begin_src emacs-lisp
    (defconst *install-dir* "~/development/jjgemacs_v2/")
...
** Control how buffers get opened overal
Still figuring this out. I want find file to consistently open on the bottom
like it always has. I want C-x 4 f to split only when there's only one window.

First few attempts failed but leavintg this so I can come back to it.
#+begin_src emacs-lisp :results silent
...
#+begin_src emacs-lisp :results silent
  (use-package compat
    :straight (compat.el :type git :host github :repo "phikal/compat.el"))
#+end_src
* General Tools
** Treemacs
#+begin_src emacs-lisp :results silent
...
#+begin_src emacs-lisp
  (use-package ob-async
    :straight (ob-async :type git :host ogithub :repo "astahlman/ob-async"))
#+end_src

*** Trying out a slack markdown export
...
Learned about this from system crafters for centering org mode text. Could work
for presentation mode (that I haven't set up yet) but I'm also curious if I want
it always on.
...
Going to give this another shot for lisp languages... moving quickly with semi
model key bindings would be nice. If this doesn't stick I'll try my own modal
that I can toggle on.
...
This is supposed to be able to be set to the symbol silent to stop popping up a
warnigns buffer, which is the behavior I would prefer. But I kept getting errors
so I set it to nil for now.
...
** Hook into magit for formatting work commit messages automatically
...
** Transparent Frame
#+begin_src emacs-lisp :results silent2
 (set-frame-parameter (selected-frame) 'alpha '(100 100))
#+end_src

** Company Backends

Force company backends to a shorter list to see if the completion issue goes
away. My current theory is that the ejc company mode was causing the occasional
delay since it was first in the list. I need to learn a bit more about this but
I've disabled ejc since I wasn't using it and shortened this list for now.
